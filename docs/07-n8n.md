...
Queue mode splits n8n into three roles: a main for the editor and API, workers that
execute, and webhook processors that receive. Postgres replaces the SQLite file, and
Valkey (Redis-compatible) holds the queue the roles hand work through. Before this: one
process, one SQLite file, a restart drops whatever was running. After this: editor,
workers, and webhook receivers in separate pods, scaling and restarting independently on
top of Postgres and a queue.
...
- Worker and webhook-processor pools, with KEDA queue-depth autoscaling (KEDA watches
  queue length and grows or shrinks the pools to match)
- PostgreSQL via CloudNativePG, a Kubernetes operator that runs and manages Postgres
  in-cluster (or an external database)
...
Terraform and OpenTofu do not share a module registry. Terraform resolves `source` against
`registry.terraform.io`, OpenTofu against `registry.opentofu.org`, and they are entirely
separate indexes. This module went to the Terraform registry first, so for a while the
README's own `source` line failed under `tofu init` with `Module not found` — a correct
config that looks broken. The fix was publishing to OpenTofu's index as well
...
Worth generalising: **a module being on one registry tells you nothing about the other.**
If a published `source` 404s, check which index your binary is asking before you touch the
config.

**Pin with a range, and make it tighter than you think.** `~> 0.1.0` is patch-only
(`>= 0.1.0, < 0.2.0`). `~> 0.1` would also accept `0.2.0`, and pre-1.0 a minor bump is
exactly where a module is still free to break its input surface. The `0.0.1-beta.*` tags
remain valid *exact* pins, but they never matched a range at all — neither Terraform nor
OpenTofu resolves a range against a pre-release. That is why `0.1.0` exists: same tree as
`0.0.1-beta.5`, retagged without the suffix so a constraint could match it.

**Turn metrics on, then turn the other 19 on too.** `n8n_metrics_enabled = true` sets
`N8N_METRICS`, plus the queue and cache groups. n8n has **23** `N8N_METRICS_INCLUDE_*`
groups and the remaining 19 default to false, so `/metrics` answers, Prometheus scrapes
happily, and nothing reports on webhooks, the scheduler, the DB pool, execution-data I/O,
SSRF blocks or DNS cache. Nothing signals the absence. Enabling all of them took one
instance from 66 to 127 metric names for roughly 1,700 extra series.
...
- **`sum()` counters, `max()` gauges.** Execution counters are per-process, so summing is
...
version you get depends on which node the pod lands on. Bump deliberately: n8n runs its
database migrations on startup and they are one-way, so rolling back to an earlier tag
against a migrated database is not a supported path.
...
task-runner metrics at all** — `dist/metrics/prometheus/` holds 23 metric services and
...
n8n ships the answer, switched off: an OpenTelemetry module that emits a `workflow.execute`
span per execution and a `node.execute` span per node.
...
through `n8n_extra_env` is rejected at plan time, along with every other name the module
manages.

What it produces, on a first traced execution — the same 8 seconds, finally attributed:
...
To graph and alert on this rather than reading traces one at a time, let Tempo's
metrics-generator convert the spans into Prometheus series — see
...
claim itself is in [`iac/tofu/n8n/storage.tf`](../iac/tofu/n8n/storage.tf); the module call wires
it in like this:
...
But it does not need to. Argo owns a `Terraform` custom resource in git (a custom
resource is a new object type a controller adds to Kubernetes, managed like any
built-in), and a controller in the cluster runs `plan`/`apply` against a module sourced
by git ref or from a registry.
...
permissions granted to it). It creates namespaces,
